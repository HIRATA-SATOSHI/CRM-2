require "application_system_test_case"

class ContactsTest < ApplicationSystemTestCase
  setup do
    @contact = contacts(:one)
  end

  test "visiting the index" do
    visit contacts_url
    assert_selector "h1", text: "Contacts"
  end

  test "creating a Contact" do
    visit contacts_url
    click_on "New Contact"

    fill_in "Content", with: @contact.content
    fill_in "Description", with: @contact.description
    fill_in "Origin", with: @contact.origin
    fill_in "Other point", with: @contact.other_point
    fill_in "Owner", with: @contact.owner
    fill_in "Priority", with: @contact.priority
    fill_in "Requirement", with: @contact.requirement
    fill_in "Response deadline", with: @contact.response_deadline
    fill_in "Responsible person", with: @contact.responsible_person
    fill_in "Staus", with: @contact.staus
    fill_in "Subject", with: @contact.subject
    click_on "Create Contact"

    assert_text "Contact was successfully created"
    click_on "Back"
  end

  test "updating a Contact" do
    visit contacts_url
    click_on "Edit", match: :first

    fill_in "Content", with: @contact.content
    fill_in "Description", with: @contact.description
    fill_in "Origin", with: @contact.origin
    fill_in "Other point", with: @contact.other_point
    fill_in "Owner", with: @contact.owner
    fill_in "Priority", with: @contact.priority
    fill_in "Requirement", with: @contact.requirement
    fill_in "Response deadline", with: @contact.response_deadline
    fill_in "Responsible person", with: @contact.responsible_person
    fill_in "Staus", with: @contact.staus
    fill_in "Subject", with: @contact.subject
    click_on "Update Contact"

    assert_text "Contact was successfully updated"
    click_on "Back"
  end

  test "destroying a Contact" do
    visit contacts_url
    page.accept_confirm do
      click_on "Destroy", match: :first
    end

    assert_text "Contact was successfully destroyed"
  end
end
